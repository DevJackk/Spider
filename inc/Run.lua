...
Spider = Token:match("(%d+)")
redis:set(Spider..":VERSION",1)
redis:set(Spider..":SUDO_ID:",SUDO_USER)
redis:set(Spider..":DataCenter:",'German')
redis:set(Spider..":UserNameBot:",BOT_User)
redis:set(Spider..":NameBot:",BOT_NAME)
redis:hset(Spider..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3Spider_INSTALL","Yes")
info = {}
...
Spider = Token:match("(%d+)")
our_id = tonumber(Spider)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(Spider..":UserNameBot:")
SUDO_ID = tonumber(redis:get(Spider..":SUDO_ID:"))
SUDO_USER = redis:hgetall(Spider..'username:'..SUDO_ID).username
version = redis:get(Spider..":VERSION")
DataCenter = redis:get(Spider..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..Spider..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(Spider..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(Spider..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(Spider..'group:add'..msg.chat_id_) then 
...
msg.TheRank =  'مالك السورس'
msg.Rank = 1
elseif msg.sender_user_id_ == 1381974291 then 
...
msg.Rank = 1
elseif redis:sismember(Spider..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(Spider..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(Spider..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(Spider..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(Spider..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(Spider..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(Spider..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(Spider..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(Spider..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(Spider..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(Spider..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(Spider..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس اسبايدر ⇐ @SouRce_Spider
...
if msg.GroupActive and redis:get(Spider..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(Spider..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(Spider..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(Spider..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.Spider then 
if msg.text and ScriptFile.iSpider then
for k, Spider in pairs(ScriptFile.Spider) do
local SearchText = msg.text:match(Spider)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iSpider(msg,{msg.text:match(Spider)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",Spider.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dSpider then
if ScriptFile.dSpider(msg) == false then
return false
end
...
if Plug.Spider then 
if msg.text and Plug.iSpider then
for k, Spider in pairs(Plug.Spider) do
local SearchText = msg.text:match(Spider)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(Spider..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(Spider..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iSpider(msg,{msg.text:match(Spider)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",Spider.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.dSpider then
Plug.dSpider(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The Source Spider")
end 

end
else
print("The File Script.lua Not Runing in The Source Spider")
...
function tdcli_update_callback(data)
	...
	redis:srem(Spider..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/jack1eljoker1/Spider/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/Spider/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/Spider/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/Spider/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/Spider/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/Spider/main/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(Spider..":SUDO_ID:")
redis:hset(Spider..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(Spider..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(Spider..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(Spider..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(Spider..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(Spider..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(Spider..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(Spider..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(Spider..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(Spider..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(Spider..'group:add'..data.chat_.id_) then
	redis:set(Spider..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(Spider..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(Spider..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(Spider..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	Spider..'group:name'..k,IDS.Title,
	Spider..'num_msg_Spider'..k,5,
	Spider..'group:add'..k,true,
	Spider..'lock_link'..k,true,
	Spider..'lock_spam'..k,true,
	Spider..'lock_webpage'..k,true,
	Spider..'lock_markdown'..k,true,
	Spider..'lock_flood'..k,true,
	Spider..'lock_bots'..k,true,
	Spider..'mute_forward'..k,true,
	Spider..'mute_contact'..k,true,
	Spider..'mute_document'..k,true,
	Spider..'mute_inline'..k,true,
	Spider..'lock_username'..k,true,
	Spider..'replay'..k,true
	)
	redis:sadd(Spider..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(Spider..'username:'..ID,'username',user)
	redis:sadd(Spider..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(Spider..'username:'..ID,'username',user)
	redis:sadd(Spider..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(Spider..'username:'..ID,'username',user)
	redis:sadd(Spider..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(Spider..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(Spider..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(Spider..'username:'..data.user_.id_).username
	...
	redis:hset(Spider..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
